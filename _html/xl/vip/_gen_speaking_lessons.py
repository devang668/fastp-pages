# -*- coding: utf-8 -*-
"""Generate speaking VIP lesson pages (lesson-NN.html) + QA pages (qa-NN.html)
from vip-ky.md. Style: xl three-column + Ctrl-K search + favicon, no emoji.
"""
import io
import os
import re

MD = r"C:/Users/Cheng/Downloads/vip-ky.md"
OUTDIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "speaking")
...
def section_text(key):
    """Return lines (including the ## header line) for section key like speak-8 / speak-09."""
...
LESSON_META = {
    1: ("speak-1", "第01讲 · 导学课：口语考试流程与评分标准", "口语考试结构、评分四维、备考心态与课程体系介绍"),
    2: ("speak-2", "第02讲 · 口语三大基本能力与基础", "流畅度、连贯性、词汇多样性的基础能力讲解与训练方法"),
    3: ("speak-3", "第03讲 · 句子结构与时态语态", "口语中句子结构转换、常用时态语态与准确表达"),
    4: ("speak-4", "第04讲 · Part One 综合练习与拓展", "Part One 常见话题、思路拓展与九大原因素材"),
    5: ("speak-5", "第05讲 · 发音练习与九大原因素材", "发音纠音、吞音习惯、九大原因素材包与串题基础"),
    6: ("speak-6", "第06讲 · Part Two 素材与串题方法", "Part Two 话题卡结构、素材准备与 3+1 回答方式"),
    7: ("speak-7", "第07讲 · 喜欢的东西：串题素材", "描述喜好类话题的素材与串题策略"),
    8: ("speak-8", "第08讲 · 成功人物素材", "成功人物类素材的构建与灵活应用"),
    9: ("speak-09", "第09讲 · 活动描述与家庭聚会素材", "活动类话题描述与家庭聚会素材"),
    10: ("speak-10", "第10讲 · 地点类素材与旅游话题", "地点类素材、旅游表达与讲解景点的方法"),
    11: ("speak-11", "第11讲 · 物品类素材：礼物/家居/衣物", "物品类素材：礼物、家居用品、衣物描述的词汇与表达"),
    12: ("speak-12", "第12讲 · 节日与有意义的一天", "节日庆祝与有意义的一天话题素材"),
    13: ("speak-13", "第13讲 · 目标 6.5 以下备考策略", "Part One / Part Two 重点备考策略与资源分配"),
    14: ("speak-14", "第14讲 · Part Three 答题思路链", "Part Three 五大类七小类答题思路链"),
    15: ("speak-15", "第15讲 · Part Three 解决类话题", "解决类话题：社会解决与个人解决两大类素材"),
    16: ("speak-16", "第16讲 · 实践与题库串讲总结", "边听边练、题库串讲与日常练习习惯总结"),
}
...
JS = """  var panel = document.getElementById('search-panel');
  ...
  panel.addEventListener('click', function(e){ var a = e.target.closest ? e.target.closest('a') : null; if (a) panel.hidden = true; });"""
...
def page(title, logo, logo_sub, left_html, main_html, right_html):
    ...
    parts.append('    <a href="speaking-index.html" style="font-size:13px;color:#2c5cdc;font-weight:600;white-space:nowrap;">口语目录</a>')
...
def left_html(active_num):
    ...
    out.append("    <ul>")
    for num in range(1, 17):
        cls = ' class="active"' if num == active_num and False else ""
        out.append('      <li><a%s href="qa-%02d.html">第%02d讲 · 问答精要</a></li>' % (cls, num, num))
    out.append("    </ul>")
    out.append('    <h3>返回</h3>')
    out.append("    <ul>")
...
def right_html(active_num, kind):
    out = []
...
def split_section(body):
    """body: lines including the ## header. Return (abstract_text, qa_pairs, transcript_paras)."""
...
def render_transcript_page(num):
    ...
    html = page(title + " · 口语VIP", "口语 VIP", "· 第 %02d 讲" % num, left_html(num), "\n".join(main), right_html(num, "lesson"))
...
def render_qa_page(num):
    ...
    html = page(title + " · 问答精要", "口语 VIP", "· 第 %02d 讲问答" % num, left_html(num), "\n".join(main), right_html(num, "qa"))
