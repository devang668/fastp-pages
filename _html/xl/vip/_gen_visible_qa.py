...
def parse_lesson(path, sub):
    ...
    for p in paras:
        ...
        if m and m.group(2).startswith("lesson-"):
            note = inline(re.sub(r"\]\(lesson-", "](" + "../../" + sub + "/lesson-", p))
            continue
        if not abstract:
            abstract = p
...
def build_qa_page(course_cn, sub, num, total, lessons):
    html = TPL_QA
    nn = "%02d" % num
    L = lessons[num - 1]
    ...
    html = html.replace('<a href="yuedu-index.html" style="font-size:13px;color:#2c5cdc;font-weight:600;white-space:nowrap;">阅读目录</a>',
                        '<a href="../../' + sub + '/' + COURSES[[c[0] for c in COURSES].index(course_cn)][2] + '" style="font-size:13px;color:#2c5cdc;font-weight:600;white-space:nowrap;">' + course_cn + "目录</a>")
    # 左侧目录
    lis = []
    ...
    for course_cn, sub, idx, total in COURSES:
        ...
        for n in range(1, total + 1):
            if n not in files:
                ...
                continue
            lessons.append(parse_lesson(files[n], sub))
        out = os.path.join(OUTROOT, sub)
        os.makedirs(out, exist_ok=True)
        for n, L in enumerate(lessons, 1):
            pg = build_qa_page(course_cn, sub, n, total, lessons)
